;!function (win) {
  ...
  Xadmin.prototype.end = function() {
	...
	Xadmin.prototype.add_tab = function (title,url,is_refresh) {
		...
		//重复点击,不会重新加载页面,即不会刷新已经打开的页面
		for (var i = 0; i <$('.x-iframe').length; i++) {
            if($('.x-iframe').eq(i).attr('tab-id')==id){
                element.tabChange('xbs_tab', id);//用于外部切换到已打开的指定的Tab项上
                if(is_refresh)
		...
		this.add_lay_tab(title,url,id); //用于新增一个Tab选项
	    this.set_data(title,url,id);
	    element.tabChange('xbs_tab', id);//用于外部切换到指定的Tab项上
